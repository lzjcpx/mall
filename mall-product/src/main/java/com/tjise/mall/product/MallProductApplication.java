 ...
 *          自动配置好了缓存换力气RedisCacheManager
 *      （2）、配置使用redis作为缓存
 *      （3）、测试使用缓存
 *          @Cacheable  触发将数据保存到缓存得操作
 *          @CacheEvict  触发将数据从缓存删除得操作
 *          @CachePut    不影响方法执行更新缓存
 *          @Caching     组合以上多个操作
 *          @CacheConfig 在类级别共享缓存得相同配置
 ...
 *          -> 就i会应用到当前RedisConfiguration管理的所有缓存分区中
 */
@EnableRedisHttpSession
@EnableFeignClients(basePackages = "com.tjise.mall.product.feign")
@EnableDiscoveryClient
@MapperScan("com.tjise.mall.product.dao")
